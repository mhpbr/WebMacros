...
window.addEventListener("load", function (event) {
    ...
    chrome.bookmarks.onChanged.addListener( function (id, x) {
        // TODO: listen to only WebMacros descendants change
...
window.addEventListener("iMacrosRunMacro", function(evt) {
    document.getElementById("webmacros-bookmark-div").setAttribute("name", evt.detail.name);
    document.getElementById("webmacros-macro-container").value = evt.detail.source;
});

function getiMacrosFolderId() {
    return new Promise((resolve, reject) => {
        chrome.bookmarks.getTree(tree => {
            // first find WebMacros subtree or create if not found
            // (code duplicates one in addToBookmarks(),
            // TODO: do something with that)
            let iMacrosFolder = tree[0].children[0].children.find(
                child => child.title == "WebMacros"
            )
            if (typeof iMacrosFolder == "undefined") {
                let bookmarksPanelId = tree[0].children[0].id
                chrome.bookmarks.create(
                    {parentId: bookmarksPanelId, title: "WebMacros"},
...
var TreeView = {
    // build tree from WebMacros bookmarks folder
    build: function () {
        ...
        chrome.bookmarks.getSubTree(id, function (treeNodes) {
            ...
            jQuery('#jstree_container').on('select_node.jstree', function (e, data) {
                ...
                if (data.node.type == 'macro') {
                    ...
                    document.getElementById("webmacros-macro-container").value = decodeURIComponent(atob(m[1]));
