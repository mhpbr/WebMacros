...
// ADD command http://wiki.webmacros.net/ADD
...
// CLEAR command http://wiki.webmacros.net/CLEAR
...
// FRAME command http://wiki.webmacros.net/FRAME
...
// IMAGESEARCH command http://wiki.webmacros.net/IMAGESEARCH
...
MacroPlayer.prototype.ActionTable["imagesearch"] = function (cmd) {
    ...
    // chrome.tabs.captureVisibleTab(this.win_id, {format: "png"}, function(_) {
        const host = "com.ipswitch.webmacros.host";
        var msg_no_free_beer = "This feature requires"+
            " the WebMacros image recognition library,"+
            " which is part of the commercial WebMacros Standard"+
...
// ONDOWNLOAD command http://wiki.webmacros.net/ONDOWNLOAD
...
MacroPlayer.prototype.ActionTable["ondownload"] = function (cmd) {
    ...
    if (folder !== "*" && !this.afioIsInstalled) {
        throw new BadParameter("FOLDER requires File Access for WebMacros Extensions. Specify FOLDER=* to save file to the browser's default download folder.");
    }
...
// ONERRORDIALOG command http://wiki.webmacros.net/ONERRORDIALOG
...
// ONLOGIN command http://wiki.webmacros.net/ONLOGIN
...
// PAUSE command http://wiki.webmacros.net/PAUSE
...
// PROMPT command http://wiki.webmacros.net/PROMPT
...
// PROXY command http://wiki.webmacros.net/PROXY
...
// REFRESH command http://wiki.webmacros.net/REFRESH
...
// SAVEAS command http://wiki.webmacros.net/SAVEAS
...
function getSaveAsFile(mplayer, folder, filename, type) {
    ...
    return f.exists().then(function(exists) {
        ...
        if (type == "extract") {
            ...
            throw new BadParameter("WebMacros for Chrome supports only "+
...
// SET command http://wiki.webmacros.net/SET
...
// SIZE command http://wiki.webmacros.net/SIZE
...
// STOPWATCH command http://wiki.webmacros.net/STOPWATCH
...
// TAG command http://wiki.webmacros.net/TAG
...
// VERSION command http://wiki.webmacros.net/VERSION
...
// URL command http://wiki.webmacros.net/URL
...
// TAB command http://wiki.webmacros.net/TAB
...
// WAIT command http://wiki.webmacros.net/WAIT
    ...
    file.exists().then(function(exists) {
        if (exists) {
            ...
            var x = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n"+
                ...
                "<Profile>\n"+
                "<!--Profiled with WebMacros for Chrome "+
